...
const SupplierManagement = () => {
  ...
  const fetchSuppliers = async () => {
    try {
      setLoading(true);
      // Use commonAPI to get all suppliers (including those created by CEO)
  ...
  const handleDelete = async (supplierId) => {
    if (window.confirm('Are you sure you want to delete this supplier?')) {
      try {
        await merchandiseManagerAPI.deleteSupplier(supplierId);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (modalMode === 'create') {
        await merchandiseManagerAPI.createSupplier(supplierData);
        setSuccess('Supplier created successfully');
      } else {
        await merchandiseManagerAPI.updateSupplier(selectedSupplier.id, supplierData);
