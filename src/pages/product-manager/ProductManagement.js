...
import { productManagerAPI, commonAPI } from '../../services/api';
...
const ProductManagement = () => {
  ...
  const fetchProducts = async () => {
    try {
      setLoading(true);
      const response = await productManagerAPI.getProducts();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (modalMode === 'create') {
        ...
        await productManagerAPI.createProduct(productData);
        setSuccess('Product created successfully');
      } else {
        await productManagerAPI.updateProduct(selectedProduct.id, productData);
